...
function movieSlugFromRequest(request) {
  const url = new URL(request.url);
  const querySlug = url.searchParams.get("slug")?.trim();
  const pathParts = url.pathname.split("/").filter(Boolean);
  const movieIndex = pathParts.lastIndexOf("movie");
  const encodedSlug = querySlug ||
    (movieIndex >= 0 ? pathParts[movieIndex + 1] : pathParts.at(-1)) ||
    "";
...
async function handleMovieSharePage(request) {
  ...
  const slug = movieSlugFromRequest(request);
  if (!slug) {
    return new Response("Không tìm thấy phim.", {
      status: 404,
  ...
  const html = renderPage({ slug, name, description, image });
...
export { handleMovieSharePage, movieSlugFromRequest, renderPage };
