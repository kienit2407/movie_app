...
class DioClient {
  static const String _disableRetryExtraKey = 'disableRetry';
  ...
  late final CacheOptions
  _cacheOptions; // khai báo biến cachingOption để áp dụng cho mọi request
  ...
  void _initDio() {
    _dio =
        Dio(); //gán trước -> lí do là do dio nếu dùng cacade thì interceptor đều khỏi tạo cùng 1 cùng nhưng mà. do là trong interceptor cần instance của dio nhưng khi khai báo như vậy nó khởi tạo không kịp gây ra lỗi. Nên giải pháp là [gán trước rồi cho nó gán từng cái base option và interceptor lần lượt]
    _dio.options = BaseOptions(
      // dùng để cấu hình các thiết lập mặc định ban đầu cho "tất cả request", giúp cần lặp lại cho từng req
      baseUrl: AppUrl.baseUrl, // đường dẫn domain
      connectTimeout: Duration(
        milliseconds: _connectTimeout,
      ), // đây là thời gian mà client kết nối đến server nếu quá timeout thì sẽ báo lỗi timeout. Ngăn ứng dụng treo, server không phản hồi, đảm bảo trải nghiệm khi có lỗi thì show ra không để đợi lâu
      receiveTimeout: Duration(
        milliseconds: _receiveTimeout,
      ), //Nếu mà đã kết nối với server rồi nhưng thời gian truyền dữ liệu be timeouted thì cũng sẽ quăng lỗi. Tránh ứng dụng bị treo và bảo vệ ứng dựng khi sever phản hồi chậm, hoặc có vấn đề
      sendTimeout: Duration(
        milliseconds: _sendTimeout,
      ), //-> thời gian gửi lên tương tự 2 cái kia
      headers: {
        //chuẩn hoá header, giảm lặp code cho mỗi req
        'Content-Type':
            'application/json', //Khi gửi đi thì baseOption này sẽ nhét vào req yêu cầu gửi đi là 1 file dạng json
        'Accept':
            'application/json', //tương tự khi get về cũng expectn là 1 dạng json báo cho server biết
      ...
      responseType: ResponseType
          .json, //định dạng dữ liệu dio mặc định là json rồi. Alternative, nó còn những dữ liệu khác byte cho file, plain cho text
      validateStatus: (status) {
        return status != null &&
            status >= 200 &&
            status < 300; // kiểm status code mà server trả về
      },
    );
    // khởi tạo lấy từ hive
    // final tokenStorage = TokenStorage();
    _dio.interceptors.addAll([
      // là một hàm để ngăn chặn, bộ lọc giữa req va res
      DioCacheInterceptor(
        options: _cacheOptions,
      ), //đặt ở đầu tiên để cache response. Đẩm bảo dữ liệu được cache nagy khi nhận từ server
      ...
      RetryInterceptor(
        //-. cái này đã đủ kích hoạt cache cho mọi request rồi
        ...
        retryEvaluator: (error, attempt) {
          //-> Chỉ retry với các lỗi network
          if (error.requestOptions.extra[_disableRetryExtraKey] == true) {
            return false;
          }
          ...
          if (error.response?.statusCode != null) {
            ...
            return statusCode >= 500 &&
                statusCode < 600; //chỉ retry khi lỗi 500 là lỗi kết nối mạng
          }
          return false;
      ...
      PrettyDioLogger(
        ...
        requestBody:
            true, // kiểm tra dữ liệu gửi lên server có đúng định dạng hay không
        ...
        compact:
            true, //loh hiển thị trẻnq dòng , không xuống dòng quá nhiều tránh khó đọc
  ...
  Future<void> initalizeCacheOption({String? baseUrl}) async {
    ...
    _cacheOptions = CacheOptions(
      ...
      store:
          MemCacheStore(), // Có thể thay bằng HiveCacheStore() cho persistent cache
      //memCache nó là mặc định của flutter (Nhưng nó sẽ lưu cache và bộ nhớ ram của thiết bị)
      // còn Hive thì nó lưu cache lâu hơn
      policy: CachePolicy
          .request, //quy định cached -> kiêm tra cache. Nếu cache còn hạn (still value) trả dữ liệu của cache còn không thì gửi request mới cho server
      hitCacheOnErrorCodes: [
        ...
        503,
        504,
      ], //nếu lỗi 500 (lỗi do server thì sẽ "sử dụng cache").Otherwise, còn nếu lỗi 401 (unauthorized -> lỗi uỷ quyền) và 403 (Forbiden) -> lỗi bị cấm
      maxStale: Duration(
        minutes: 1,
      ), // cái này set chung cho mọi request nhưng tuỳ call api thì thời gian sẽ khác có thể ghi đè lai
      priority: CachePriority
          .normal, // về mức độ ưu tiên nếu normal thì nếu ram đầy thì nó sẽ tự động xoá đi
      cipher:
          null, //mã hoá dữ liệu, nếu muôn hoá dữ liệu để lưu vào cache có thể sử dụng encrypt. Null -> lưu dưới dạng plain text. cái này dùng cho token
      keyBuilder: CacheOptions
          .defaultCacheKeyBuilder, //tạo khoá key cho cached dauw vào url và query paramaters của request, đảm bảo mỗi request là duy nhất và giúp dio truy xuất đúng
  ...
  Dio get dio =>
      _dio; // getter để có thể truy cập dio từ lớp bên ngoài (readOnly not set because we can not replace any data in this class) .. nếu các lớp bên ngoài truy cập tuỳ tiện từ bên ngoài có thể làm hỏng config
  ...
  Future<Response> get({
    ...
    try {
      final requestOption = retryOnFailure
          ? option
          : (option ?? Options()).copyWith(
              extra: {...?option?.extra, _disableRetryExtraKey: true},
            );
      final response = await _dio.get(
        path,
        options: requestOption, // để overide option
  ...
  Future<Response> post(
    ...
    try {
      final response = await _dio.post(
        ...
        options: option,
        queryParameters:
            queryParameters, // ví dụ post lên dựa theo userid chẳng hạn
      );
      return response;
    } on DioException catch (e) {
      throw _handlingErrol(e);
